...
lpm_t *lpm_create() {
  return (lpm_t *) calloc(sizeof(lpm_t), 1);
}

int lpm_set_prefix_port(lpm_t *lpm,
                         uint32_t ip, int prefix_len,
                         int port) {
  prefix_node_t **node = lpm;

  // Explore prefix match tree up to the prefix depth.
  for (int pos = 0; pos <= prefix_len; pos++) {
    // Create new nodes if needed.
    if (!(*node)) {
      *node = (prefix_node_t *) malloc(sizeof(prefix_node_t));
...
int lpm_get_ip_port(lpm_t *lpm, uint32_t ip, uint32_t *out_port) {
  ...
  for (int pos = sizeof(ip) * 8 - 1; pos >= 0 && node; pos--) {
    if (node->port != NULL_PORT) {
      port = node->port;
    }
  ...
  *lpm_out = lpm_create();
  ...
  for (unsigned long count = 0; count < LPM_MAX_RULES; ++count) {
    ...
    if (asn < 0 || nb_devices < asn) {
      ...
      result = lpm_set_prefix_port(*lpm_out,
                                   rte_be_to_cpu_32(ip), depth, if_out);
      if (result < 0) {
        rte_exit(EXIT_FAILURE,
...
{
  uint32_t next_hop;
  int success = lpm_get_ip_port(lpm,
