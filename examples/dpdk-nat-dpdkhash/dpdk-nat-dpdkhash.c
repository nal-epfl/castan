...
uint32_t castan_rte_jhash(const void *key, uint32_t key_len,
                          uint32_t init_val) {
  uint32_t hash;
  castan_havoc(*((hash_key_t *) key), hash, rte_jhash(key, key_len, init_val));
  return hash;
}

void initialize(struct nf_config *config, struct rte_hash **hash_table) {
  ...
  struct rte_hash_parameters p = {
      ...
      .entries = 128 * 1024,
      .reserved = 0,
      .key_len = sizeof(hash_key_t),
      .hash_func = castan_rte_jhash,
      .hash_func_init_val = 0,
      .socket_id = 0,
      .extra_flag = 0,
  };
  *hash_table = rte_hash_create(&p);
}

uint32_t dispatch_packet(struct nf_config *config, uint32_t device,
                         struct rte_hash *hash_table, struct rte_mbuf *mbuf) {
  ...
  hash_value_t *translation = NULL;
  if (rte_hash_lookup_data(hash_table, &key, (void **)&translation) ==
      -ENOENT) {
    ...
    if (ip->dst_addr == config->nat_ip) {
      ...
      // Find unused port.
      hash_key_t *out_key = (hash_key_t *)malloc(sizeof(hash_key_t));
      out_key->src_ip = ip->dst_addr;
      out_key->dst_ip = config->nat_ip;
      out_key->proto = ip->next_proto_id;
      out_key->src_port = dport;
      out_key->dst_port = sport;

      for (; rte_hash_lookup(hash_table, out_key) > 0; out_key->dst_port++) {
      }
      ...
      hash_key_t *new_key = (hash_key_t *)malloc(sizeof(hash_key_t));
      *new_key = key;

      translation = (hash_key_t *)malloc(sizeof(hash_key_t));
      *translation = key;
      translation->src_ip = config->nat_ip;
      translation->src_port = out_key->dst_port;
      rte_hash_add_key_data(hash_table, new_key, translation);

      // Save entry for returning traffic.
      hash_key_t *out_translation = (hash_key_t *)malloc(sizeof(hash_key_t));
      *out_translation = *out_key;
      out_translation->dst_ip = ip->src_addr;
      out_translation->dst_port = sport;
      rte_hash_add_key_data(hash_table, out_key, out_translation);
    }
  }
...
void run(struct nf_config *config, struct rte_hash *hash_table) {
  while (1) {
    for (uint32_t device = 0; device < NUM_ETHPORTS; ++device) {
