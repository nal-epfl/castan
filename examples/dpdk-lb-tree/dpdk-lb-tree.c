...
typedef struct __attribute__((packed)) hash_key_t {
  ...
  bool operator==(const hash_key_t &other) const {
    return this->src_ip == other.src_ip && this->proto == other.proto &&
           this->src_port == other.src_port;
  }
  bool operator!=(const hash_key_t &other) const { return !((*this) == other); }
} hash_key_t;
...
void hash_set(hash_table_t hash_table, hash_key_t key, hash_value_t value) {
  ...
  if (!*pos) {
    *pos = (tree_node_t *)calloc(1, sizeof(tree_node_t));
    (*pos)->key = key;
  }
  (*pos)->value = value;
}
