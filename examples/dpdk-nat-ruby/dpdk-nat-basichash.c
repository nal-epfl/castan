...
typedef struct st_table *hash_table_t;

int hash_key_equals(st_data_t a, st_data_t b) {
  return (((hash_key_t *)&a)->src_ip == ((hash_key_t *)&b)->src_ip) &
         (((hash_key_t *)&a)->dst_ip == ((hash_key_t *)&b)->dst_ip) &
         (((hash_key_t *)&a)->proto == ((hash_key_t *)&b)->proto) &
         (((hash_key_t *)&a)->src_port == ((hash_key_t *)&b)->src_port) &
         (((hash_key_t *)&a)->dst_port == ((hash_key_t *)&b)->dst_port);
}
...
st_index_t hash_function(st_data_t key) {
  // Based on Bob Jenkins' lookup3 algorithm.
  uint32_t a, b, c;

  a = b = c = 0xdeadbeef + ((uint32_t)sizeof(hash_key_t));

  a += ((hash_key_t *)&key)->src_ip;
  b += ((hash_key_t *)&key)->dst_ip;
  c += ((uint32_t)((hash_key_t *)&key)->src_port) << 16 |
       ((hash_key_t *)&key)->dst_port;
  hash_function_mix(a, b, c);

  a += ((hash_key_t *)&key)->proto;

  hash_function_final(a, b, c);
  return c;
}

st_index_t havoced_hash_function(st_data_t key) {
  uint32_t hash;
  castan_havoc(key, hash, hash_function(key));
  return hash;
}

void hash_init(hash_table_t *hash_table) {
  static struct st_hash_type ruby_hashcmp;
  ruby_hashcmp.compare = &hash_key_equals;
  ruby_hashcmp.hash = &havoced_hash_function;
  // I checked the source, st_init_table just ignores malloc errors...
  *hash_table = st_init_table_with_size(&ruby_hashcmp, (st_index_t)TABLE_SIZE);
}

void hash_set(hash_table_t hash_table, hash_key_t key, hash_value_t value) {

  st_data_t st_key = 0;
  memcpy(&st_key, &key, sizeof(key));
  st_data_t st_value = 0;
  memcpy(&st_value, &value, sizeof(value));
  st_insert(hash_table, st_key, st_value);
}
...
int hash_get(hash_table_t hash_table, hash_key_t key, hash_value_t *value) {
  ...
  memcpy(&st_key, &key, sizeof(key));
  st_data_t st_value;
  if (st_lookup(hash_table, st_key, &st_value)) {
    if (value) {
      *value = *((hash_key_t *)&st_value);
    }
    return 1;
  }
  return 0;
