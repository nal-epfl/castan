...
typedef struct __attribute__((packed)) hash_key_t {
  ...
  bool operator==(const hash_key_t &other) const {
    return (this->src_ip == other.src_ip) & (this->dst_ip == other.dst_ip) &
           (this->proto == other.proto) & (this->src_port == other.src_port) &
...
namespace std {
template <> struct hash<hash_key_t> {
  std::size_t operator()(const hash_key_t &key) const {
  uint32_t hash;
...
typedef std::unordered_map<hash_key_t, hash_value_t> *hash_table_t;

void hash_init(hash_table_t *hash_table) {
  *hash_table = new std::unordered_map<hash_key_t, hash_value_t>();
}
