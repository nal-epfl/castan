...
void hash_init(hash_table_t *hash_table) {
  const struct rte_memzone *mz = rte_memzone_reserve_aligned(
      "HASHRING", TABLE_SIZE * sizeof(hash_entry_t), rte_socket_id(), RTE_MEMZONE_1GB, PAGE_SIZE);
  if (!mz) {
    rte_exit(EXIT_FAILURE, "Unable to allocate hash ring.\n");
  }

#ifndef __clang__
  memset(mz->addr, 0, TABLE_SIZE * sizeof(hash_entry_t));
#endif

  NF_INFO("Hash ring physical address: %016lX\n", rte_mem_virt2phy(mz->addr));

  *hash_table = (hash_table_t)mz->addr;
}
