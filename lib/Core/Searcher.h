...
namespace castan {
  ...
  private:
    ...
    std::map<const llvm::Instruction *, std::pair<bool, double>> costs;
    std::map<const llvm::Instruction *, double> successorCosts;

    std::vector<double> getPriority(klee::ExecutionState *state);
